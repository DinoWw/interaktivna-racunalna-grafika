...
int main(int argc, char** argv)
{
	...
	faceVerts = (Triangle*) malloc(3 * faceCount * sizeof(Triangle));
	vertCount = 0;
	faceCount = 0;
	...
	while( fgets(line, sizeof(line), file) != NULL ) {
		...
		case('f'):
			...
			faceVerts[faceCount] = {vertIds[0], vertIds[1], vertIds[2]};
	...
	int *facesAdj = (int *)malloc(vertCount*sizeof(int));
	...
	for(int f = 0; f < faceCount; f+=1) {
		for(int i = 0 ; i < 3; i ++){
			facesAdj[faceVerts[f].vertIds[i]] += 1;
	
			Vec *v = &vNormals[faceVerts[f].vertIds[i]];
			v->x += fNormals[f].x;
			v->y += fNormals[f].y;
			v->z += fNormals[f].z;
		}
	...
	for(int i = 0; i < vertCount; i++) {
		vNormals[i].x /= facesAdj[i];
		vNormals[i].y /= facesAdj[i];
		vNormals[i].z /= facesAdj[i];
	}


	// ucitaj koordinate izvora svjetlosti
	printf("Unesi koordinate izvora (x y z):\n");
	scanf("%lf %lf %lf", &izvorVec.x, &izvorVec.y, &izvorVec.z);
...
Vec normalized(Vec v) {
	GLdouble len = sqrt(v.x*v.x + v.y*v.y + v.z*v.z);
	return {
		v.x/len,
		v.y/len,
		v.z/len
...
//	glutSolidTeapot (1.0);
	...
	static double ka = 0.3;
	static double Ia = 100;
	static double kd = 0.7;
	static double Ii = 100;
	...
	for(int f = 0; f < faceCount; f+=1) {
		...
		Vec N;
		Vec L = normalized(izvorVec);

		GLdouble intensity;
		if(vertShading) {
			N = normalized(vNormals[faceVerts[f].vertIds[0]]);
			intensity = Ia*ka + Ii*kd * fmax(0, (N.x*L.x + N.y*L.y + N.z*L.z));
			glColor3ub(intensity, intensity, intensity);
			glVertex3d(faces[3*f][0], faces[3*f][1], faces[3*f][2]);
	
			N = normalized(vNormals[faceVerts[f].vertIds[1]]);
			intensity = Ia*ka + Ii*kd * fmax(0, (N.x*L.x + N.y*L.y + N.z*L.z));
			glColor3ub(intensity, intensity, intensity);
			glVertex3d(faces[3*f+1][0], faces[3*f+1][1], faces[3*f+1][2]);
	
			N = normalized(vNormals[faceVerts[f].vertIds[2]]);
			intensity = Ia*ka + Ii*kd * fmax(0, (N.x*L.x + N.y*L.y + N.z*L.z));
			glColor3ub(intensity, intensity, intensity);
...
void myKeyboard(unsigned char theKey, int mouseX, int mouseY)
{
	switch (theKey) 
	{
		...
		case 'v': vertShading = !vertShading;
        break;
